...
impl From<nom::Err<Self>> for ErrorKind {
  fn from(error: nom::Err<Self>) -> Self {
    match error {
...
impl Metric {
  ...
  fn parse_escaped_string(input: &str) -> IResult<'_, String> {
    ...
    fn match_quote(input: &str) -> IResult<'_, char> {
      char('"')(input).map_err(|_: NomError<'_>| {
...
impl Header {
  ...
  fn parse(input: &str) -> IResult<'_, Self> {
    let input = trim_space(input);
    let (input, _) = char('#')(input).map_err(|_: NomError<'_>| ErrorKind::ExpectedChar {
      expected: '#',
      input: input.to_owned(),
    })?;
    let input = trim_space(input);
    let (input, _) = tag("TYPE")(input).map_err(|_: NomError<'_>| ErrorKind::ExpectedToken {
      expected: "TYPE",
      input: input.to_owned(),
    })?;
    ...
    .map_err(|_: NomError<'_>| ErrorKind::InvalidMetricKind {
      input: input.to_owned(),
    })?;
    Ok((input, Self { metric_name, kind }))
  }
}
...
impl Line {
  ...
  pub(crate) fn parse(input: &str) -> Result<Option<Self>, ErrorKind> {
    ...
    let metric_error = match Metric::parse(input) {
      Ok((_, metric)) => {
        return Ok(Some(Self::Metric(metric)));
      },
      Err(e) => e.into(),
    };
    ...
    let header_error = match Header::parse(input) {
      Ok((_, header)) => {
        return Ok(Some(Self::Header(header)));
      },
      Err(e) => e.into(),
    };

    if let Ok((input, _)) = char::<_, NomErrorType<'_>>('#')(input) {
      if tuple::<_, _, NomErrorType<'_>, _>((sp, tag("TYPE")))(input).is_ok() {
...
fn parse_name(input: &str) -> IResult<'_, String> {
  ...
  .map_err(|_: NomError<'_>| ErrorKind::ParseNameError {
    input: input.to_owned(),
  })?;
...
fn match_char(c: char) -> impl Fn(&str) -> IResult<'_, char> {
  move |input| {
    preceded(sp, char(c))(input).map_err(|_: NomError<'_>| {
      ErrorKind::ExpectedChar {
        expected: c,
        input: input.to_owned(),
      }
